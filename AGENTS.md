...
## Command Output Filtering (Windows/PowerShell)

When running commands in this Windows environment, use PowerShell cmdlets for filtering output:

```powershell
# Get last N lines (instead of Unix 'tail')
Your-Command | Select-Object -Last N

# Get first N lines (instead of Unix 'head')
Your-Command | Select-Object -First N

# Examples:
dotnet build | Select-Object -Last 20
dotnet test 2>&1 | Select-Object -First 10
```

Do NOT use Unix tools like `tail`, `head`, or `grep`. Use PowerShell equivalents instead.
